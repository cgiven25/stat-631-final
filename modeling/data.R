...
fit <- glm(MadePlayoffs ~ AvAge + SOS + cap_space + ltir, family = "binomial", data = train)
...
# 
fit.points <- lm(PTS ~ AvAge + SOS + cap_space, data = train)
plot(predict(fit.points), train[complete.cases(train), "PTS"])
abline(0, 1)

# we tried it yay
div.from.pts <- multinom(division ~ PTS, data = train)

# these plus PP and PK?
t <- multinom(W.bin ~ AvAge + SOS + cap_space + ltir, data = train)

# ordinal logistic regression: future work
t2 <- polr(W.bin ~ AvAge + SOS + division, data = train)
